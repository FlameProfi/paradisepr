...
namespace NeptuneEvo
{
    ...
    {
        public static string Codename { get; } = "Revolution Classic";
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                if (ServerNumber <= 9) // Поставлены только на Black'е и отображать на Test'овом
                {
            ...
            try
            {
                ...
                switch (door.Split('_')[0])
                {
                    case "pd":
                        ...
                        break;
                    case "jail":
                        if (fractionId != (int)Fractions.Models.Fractions.PRISON && fractionId != (int)Fractions.Models.Fractions.FIB) return;
                        else if (!player.IsFractionAccess(RankToAccess.DoorControl)) return;
                        break;
                    case "fib":
